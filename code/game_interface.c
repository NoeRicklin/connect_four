...
int test_win(int col, int player) {
    ...
    while (game_state[row*board_width + col] == EMPTY_TILE) { row--; }
...
int _test_hor_win(int row, int player) {
	...
	if (game_state[row*board_width + stone_index] == player) {
	    counter++;
...
int _test_vert_win(int col, int player) {
    ...
    for (int stone_index = 0; stone_index < board_width; stone_index++) {
	if (game_state[stone_index*board_width + col] == player) {
	    counter++;
...
int _test_neg_diag_win(int row, int col, int player) {
	...
	for (int stone_index = 0;
	     ...
	     stone_index++)
	{
	    if (game_state[current_stone_index] == player) {
		counter++;
...
void print_game_state() {    
	...
	for (int col = 0; col < board_width; col++) {
	    switch (game_state[row*board_width + col]) {
		case PLAYER_ONE: {
		    printf("x");
		} break;
		case PLAYER_TWO: {
